...
const student = ApiStudent(
  ...
  classUuid: 'c',
  sectionUuid: 'a',
  admissionNo: 'ADM-2026-123',
  fullName: 'Élodie D’Souza – Hernández',
  isActive: true,
  bloodGroup: 'B+',
  photoPath: 'photo.png',
...
const school = SchoolProfile(
  uuid: 'school',
  schoolCode: 'SCH',
  schoolName: 'École International',
...
DesignDocument fixture(double width, double height, {bool images = true}) {
  DesignElement text(
    ...
    id: id,
    type: static ? DesignElementType.text : DesignElementType.boundText,
    x: x ?? 3,
    y: y,
    width: w ?? width - 6,
    height: 8,
    zIndex: 2,
    data: static ? {'text': field} : {'field': field},
    style: {
      'font_size': 2.6,
      'font_weight': weight,
      'alignment': align,
  ...
  return DesignDocument(
    ...
    elements: [
      ...
      DesignElement(
        ...
        height: 2,
        rotation: 25,
        style: {'color': '#D04080', 'border_width': .5},
      ),
      text(
        'hidden',
        1,
...
void main() {
  ...
  test(
    'shared scene resolves all binding families, visibility and paint units',
    () {
      ...
      expect(node('header').style.borderWidth, .5);
      expect(node('header').style.radius, 2);
      expect(PdfDocumentRenderer.rotation(node('line')), -node('line').radians);
      for (final n in scene.elements) {
        expect(
          identical(
            n.element,
            doc.elements.firstWhere((e) => e.id == n.element.id),
          ),
          true,
        );
      }
      for (final weight in DesignFonts.names.keys) {
        expect(
          DesignRenderStyle(
            doc.elements.first.copyWith(style: {'font_weight': weight}),
          ).weight,
          weight,
        );
      }
  ...
  test(
    'PDF line layout preserves explicit newlines, long values and baselines',
    () {
      for (final align in ['left', 'center', 'right']) {
        final element = DesignElement(
          ...
          style: {'font_size': 3, 'alignment': align, 'max_lines': 3},
        );
        final node = DesignRenderElement(
          element,
          element.data['text'] as String,
          null,
        );
        ...
        for (var i = 1; i < lines.length; i++) {
          expect(lines[i].baseline - lines[i - 1].baseline, closeTo(3, .05));
        }
        expect(lines.every((l) => l.x >= 0), true);
        final long = DesignRenderElement(
          element.copyWith(width: 12),
  ...
  test(
    ...
    () async {
      expect(
        () => DesignFonts.validatePdfText('विद्यालय विद्यार्थी'),
        throwsUnsupportedError,
      );
      expect(
        () => DesignFonts.validatePdfText('Élodie • São Paulo – “ID”'),
        returnsNormally,
      );
      final doc = fixture(53.98, 85.6, images: false);
      final hindi = doc.copyWith(
        elements: [
          DesignElement(
            id: 'hindi',
            type: DesignElementType.text,
      ...
      await expectLater(
        PdfService.generateStudentCard(
          student: student,
          schoolName: 'School',
          template: CardTemplate(name: 'Hindi', document: hindi),
        ),
        throwsUnsupportedError,
      );
    },
  ...
  for (final size in [
    const Size(53.98, 85.6),
    const Size(85.6, 53.98),
    const Size(100, 100),
  ]) {
    testWidgets(
      ...
      (t) async {
        ...
        for (var x = 0; x < 60; x++) {
          for (var y = 0; y < 30; y++) {
            image.setPixelRgb(x, y, x < 30 ? 230 : 20, x < 30 ? 30 : 160, 80);
          }
        }
        final raw = img.encodePng(image);
        final memory = pw.MemoryImage(raw);
        final previousImageClientProvider = debugNetworkImageHttpClientProvider;
        ...
        try {
          ...
          pdf.addPage(
            pw.Page(
              pageFormat: PdfPageFormat(
                size.width * PdfPageFormat.mm,
                size.height * PdfPageFormat.mm,
              ),
          ...
          expect(source.contains('/ca 0.8'), true);
          expect(source.contains('/BaseFont /Helvetica'), false);
          // Match the PDF helper's document-unit layout scale. Line heights are
          // rounded by the text engine, so baselines are not scale invariant.
          ...
          for (final node in scene.elements.where(
            ...
            final lines = layoutDesignText(node);
            // RenderBox.getDistanceToBaseline is only valid during layout/paint.
            // Measure the actual widget's text and constraints independently of the
            // PDF helper, retaining the rendered offset within the element.
            final painter = TextPainter(
              ...
              ellipsis: paragraph.overflow == TextOverflow.ellipsis
                  ? '\u2026'
                  : null,
            ...
            try {
              ...
              expect(
                paragraph.size,
                paragraph.constraints.constrain(painter.size),
              );
              final metrics = painter.computeLineMetrics();
              expect(metrics, hasLength(lines.length), reason: node.element.id);
              for (var i = 0; i < lines.length; i++) {
                final reason = '${node.element.id}, line $i, fixture $size';

                expect(
                  lines[i].baseline,
                  closeTo((rect.dy - origin.dy + metrics[i].baseline), .002),
                  reason: reason,
                );
                expect(
                  lines[i].x,
                  closeTo((rect.dx - origin.dx + metrics[i].left), .002),
                  reason: reason,
                );
              }
            } finally {
          ...
          // Flutter checks painting globals before addTearDown callbacks run.
